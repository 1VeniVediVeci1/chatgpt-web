...
router.get('/chatrooms-count', auth, async (req, res) => {
  try {
    ...
    rooms.data.forEach((r) => {
      result.push({ uuid: r.roomId, title: r.title, userId: r.userId, name: r.username, lastTime: formatTimestamp(r.dateTime), chatCount: r.chatCount })
    })
...
router.post('/session', async (req, res) => {
  try {
    ...
    if (userId != null) {
      ...
      count.forEach((c) => {
        const thisChatModel = visibleChatModelOptions.filter(d => d.value === c.key)[0]
        if (!thisChatModel) return
        const suffix = c.count > 1 ? ` (${c.count})` : ''
        chatModels.push({ label: `${thisChatModel.label}${suffix}`, key: c.key, value: c.key })
      })
